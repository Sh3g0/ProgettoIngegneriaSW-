  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.user && data.user.ruolo) router.push('/home');
      }
    } catch (error) {
    ...
    <div className="login-container">
      ...
      <div className="right">
        ...
        <div className="input-group" style={{ position: 'relative' }}>
          ...
          <input id="password" type={showPassword ? 'text' : 'password'} placeholder="Inserisci la tua password" className="login-input" />
          <button type="button" className="toggle-password" onClick={() => setShowPassword(prev => !prev)}
            style={{ position: 'absolute', right: '10px', top: '36px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.2rem' }}>
            {showPassword ? '🙈' : '👁️‍🗨️'}
          </button>
        </div>
        ...
        <div className="signup">Don’t have an account? <a href="/registrazione">Sign up</a></div>
      </div>
    </div>
  );
}