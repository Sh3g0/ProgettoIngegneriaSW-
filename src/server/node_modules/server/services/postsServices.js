...
async function getRoleByUsernameAndPassword(username, password) {
  ...
  console.log('lebrooooooon');  
  if (result.length > 0) {
    return result[0].ruolo;  //Restituisce il ruolo se trovato
  } else {
    return "";  //Restituisce un array vuoto se non trovato
  }
}

// Funzione per inserire un nuovo cliente
async function addClient(email, username, passwordHash, ruolo = "utente", idAgenzia = null) {
    const query = `
